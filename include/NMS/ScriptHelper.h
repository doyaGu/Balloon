#ifndef NMS_SCRIPTHELPER_H
#define NMS_SCRIPTHELPER_H
...
namespace scripthelper {
    inline bool FindBB(CKBehavior *script, const std::function<bool(CKBehavior *)> &callback, const char *name = nullptr, bool hierarchically = false,
    ...
    inline CKBehavior *FindFirstBB(CKBehavior *script, const char *name = nullptr, bool hierarchically = false,
        ...
        FindBB(script, [&res](CKBehavior *beh) {
                   res = beh;
                   return false;
               },
    ...
    inline CKBehaviorLink *CreateLink(CKBehavior *script, CKBehavior *inBeh, CKBehavior *outBeh, int inPos = 0, int outPos = 0, int delay = 0) {
        return CreateLink(script, inBeh->GetOutput(inPos), outBeh->GetInput(outPos), delay);
    }

    inline CKBehaviorLink *CreateLink(CKBehavior *script, CKBehavior *inBeh, CKBehaviorIO *out, int inPos = 0, int delay = 0) {
        return CreateLink(script, inBeh->GetOutput(inPos), out, delay);
    }

    inline CKBehaviorLink *CreateLink(CKBehavior *script, CKBehaviorIO *in, CKBehavior *outBeh, int outPos = 0, int delay = 0) {
    ...
    inline CKBehavior *FindPreviousBB(CKBehavior *script, CKBehavior *beh, const char *name = nullptr,
                                      int inPos = -1, int outPos = -1, int inputCnt = -1, int outputCnt = -1,
                                      int inputParamCnt = -1, int outputParamCnt = -1) {
        CKBehaviorLink *link = FindPreviousLink(script, beh, name, inPos, outPos, inputCnt, outputCnt, inputParamCnt,
                                                outputParamCnt);
    ...
    inline void DeleteLink(CKBehavior *script, CKBehaviorLink *link) {
        ...
        if (!nop) {
            ...
            nop->InitFromGuid(CKGUID(0x302561c4, 0xd282980)); // LOGICS_NOP
